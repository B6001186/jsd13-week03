...
function getUnavailableItems(items) {
  const result = items.find(items => items.isAvailable === false)
  return result;
...
function getAffordableItems(items) {
  const result = items.filter(items => items.price < 200)
  return result;
}
...
function getLowStockItems(items) {
  const result = items.filter(items => items.stock < 10)
  return result;
}
...
function getCompletedOrders(orderList) {
  const result = orderList.filter(orderList => orderList.status === 'completed')
  return result;
}
...
function getAvailableBurgers(items) {
  const result = items.filter(items => items.category === 'burger' && items.isAvailable === true)
  return result;
}
...
function getBurgersWithinBudget(items, maximumPrice) {
  const result = items.filter(items => items.price <= maximumPrice)
  return result;
}
